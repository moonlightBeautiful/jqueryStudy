    ...
    lhgdialog.fn = lhgdialog.prototype =
        {
            ...
            _iframe: function (url) {
                ...
                that._frmTimer = setTimeout(function () {
                    that._postframe(url, config.urlParam, config.id);
                    /*var _urlParam = "";
                    var _index = 1;
                    for (var _name in config.urlParam) {
                        if (_index === 1) {
                            _urlParam = _urlParam + _name + "=" + config.urlParam[_name];
                        } else {
                            _urlParam = _urlParam + "&" + _name + "=" + config.urlParam[_name];
                        }
                        _index++;
            ...
            /*! 为request.getParameter迟加载iframe的src属性，IE6下不延迟 */
            _postframe: function (_url, _params, _frameid) {
                var tempForm = _doc.createElement("form");
                tempForm.action = _url;
                tempForm.method = "post";
                tempForm.target = _frameid;
                tempForm.style.display = "none";
                for (var _name in _params) {
                    var opt = _doc.createElement("textarea");
                    opt.name = _name;
                    opt.value = _params[_name];
                    tempForm.appendChild(opt);
                }
                _doc.body.appendChild(tempForm);
                tempForm.submit();
                $(tempForm).remove();
                return false;
            }
